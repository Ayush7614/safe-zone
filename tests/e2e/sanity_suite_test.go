...
func TestSanity_DetectAndGatewayBasicFlow(t *testing.T) {
	...
	t.Run("gateway-safe", func(t *testing.T) {
		...
		if err != nil {
			// If error is 502 (upstream unreachable), it's acceptable for this test environment
			if apiErr, ok := err.(*tszclient.APIError); ok && apiErr.StatusCode == 502 {
				t.Logf("ChatCompletions returned 502 (upstream unreachable), which is expected in test environment without live LLM. Error: %v", err)
				return
			}
